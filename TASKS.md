...
- [x] `sub_agents/planner.md` — Planner 프롬프트
...
- [x] `wiki_builder/post_plan.py` 구현
...
- [x] `sub_agents/post_plan.md` 생성
- [x] `orchestrate.py` — plan → post_plan → generate 순서로 연결
- [x] 테스트:
  ```bash
...
- [x] `wiki_builder/evaluate.py` — `run_evaluate()` 구현
  - 원인 분석 LLM 호출
  - sub_agents/*.md 개선안 도출
  - 개선안으로 검증 재실행
  - 보고서 생성
  - 사람 컨펌 대기 (터미널 입력)
- [x] eval.log 포맷 확인
...
- [x] 소스 2개 이상으로 전체 실행
  ...
  ```
- [x] plan.json → wiki 생성 → 링크 → 평가 전체 흐름 확인
- [x] 중간 중단 후 재실행으로 재개 확인

---

## Task 9: Phase 5 (Query) 구현 ✅

Karpathy 패턴: 질문 → index.md 검색 → 관련 페이지 읽기 → 합성 → 선택적 filing

- [x] `wiki_builder/query.py` 구현
...
- [x] `sub_agents/query_selector.md`, `sub_agents/query_synthesizer.md` 생성
- [x] filing 옵션: 답변을 `wiki/query/YYYY-MM-DD_topic.md`로 저장
- [x] `wiki/log.md`에 `## [날짜] query | 질문` 형식으로 append
- [x] `orchestrate.py` — `--phase query --question "..."` 연결
- [x] 테스트:
  ```bash
...
## Task 10: Phase 6 (Lint) 구현 ✅

Karpathy 패턴: 주기적 wiki 건강 검진

- [x] `wiki_builder/lint.py` 구현
  ...
  - LLM 호출 항목 (페이지 묶음 단위):
    ...
    - 오래된 주장 감지
    - 데이터 공백 제안
- [x] `sub_agents/lint.md` 생성
- [x] 리포트를 `wiki/lint_YYYY-MM-DD.md`로 저장
- [x] `wiki/log.md`에 lint 결과 기록
- [x] `orchestrate.py` — `--phase lint` 연결
- [x] 테스트:
  ```bash
...
## Task 11: Chat REPL 구현 ✅

터미널 대화형 인터페이스. LLM Wiki의 human-facing 레이어.

- [x] `wiki_builder/chat.py` 구현
...
- [x] `orchestrate.py` — `--phase chat` 연결
- [x] 테스트:
  ```bash
...
## Task 12: index.md / log.md 자동 관리 ✅

Generate/Query/Lint 결과가 index.md와 log.md에 반영되도록.

- [x] `wiki_builder/index_manager.py` 구현 (또는 `orchestrate.py`에 통합)
  - `update_index(wiki_dir)`: 전체 wiki 스캔 후 index.md 재생성
  - `append_log(wiki_dir, entry: str)`: log.md에 한 줄 추가
    - 형식: `## [YYYY-MM-DD] {type} | {title}`
- [x] Generate phase 완료 후 `update_index()` 자동 호출
- [x] Query/Lint 실행 시 `append_log()` 자동 호출

---

## Task 13: Server 모드 구현 (sdmAnalyzer 연동 핵심) ✅

JSON stdio 프로토콜 서버. GUI 프로그램이 subprocess로 spawn하여 통신.

- [x] `wiki_builder/server.py` 구현
  ...
  - stdin EOF → 정상 종료
- [x] `orchestrate.py` — `--phase server` 연결
- [x] 테스트 (터미널에서 수동):
  ```bash
...
## Task 14: WikiClient 구현 (sdmAnalyzer에서 import) ✅

sdmAnalyzer가 import하는 클라이언트 래퍼. llmwiki venv의 서버 프로세스를 관리.

- [x] `wiki_builder/wiki_client.py` 구현
...
- [x] 사용 예시 (sdmAnalyzer 측):
  ...
  client.stop()
  ```
- [x] 테스트: wiki server 실제 spawn 후 query 응답 확인

---

## Task 15: orchestrate.py 리팩토링 — LLM Orchestrator + tool 구조 ✅

현재 Python 함수 파이프라인을 LLM 에이전트가 tool을 호출하는 구조로 전환.

- [x] `orchestrate.py` — Orchestrator LLM 에이전트 루프 구현
  - 사용자 요청(--phase 인자)을 받아 Orchestrator LLM에게 전달
  - Orchestrator가 tool 호출로 각 Phase 실행 결정
- [x] 각 Phase를 tool definition으로 wrapping
...
- [x] tool 결과를 Orchestrator가 받아 다음 action 결정
